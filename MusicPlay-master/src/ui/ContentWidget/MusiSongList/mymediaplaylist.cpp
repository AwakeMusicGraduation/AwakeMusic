...
void MyMediaPlayList::initForm()
{
    ...
    m_table1->setShowGrid(true);//display the grid
    m_table1->setEditTriggers(QAbstractItemView::NoEditTriggers);     //设置表格不能编辑
    m_table1->setSelectionBehavior(QAbstractItemView::SelectRows);    //设置整行选中
    m_table1->setSelectionMode(QAbstractItemView::SingleSelection);
    m_table1->verticalHeader()->setDefaultSectionSize(45);
...
void MyMediaPlayList::addContenItem(QString content)
{
    qDebug()<<content;
...
void MyMediaPlayList::slotReceiveList1(QMap<int, QString>  &musicname)
{
    qDebug()<<"接收到默认播放列表";
    removeAllItem();
    QMap<int, QString>::const_iterator i = musicname.constBegin();
    while(i!=musicname.constEnd()){
        QString s=i.value();
        qDebug()<<s;
        addContenItem(s);
        ++i;
